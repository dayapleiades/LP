...
int main() { //Programa principal
  ...
  int v[9][9]; // Declarando matriz 9x9
  ...
  while (count <= ni) { // Enquanto contador for menor ou igual o número de instâncias faça
    for (i = 0; i < 9; i++) { // Número da linha
      for (j = 0; j < 9; j++) { // Número da coluna
        scanf("%d", &v[i][j]); // Lê um valor para a posição i linhas e j colunas da matriz
      }
    }
    for (i = 0; i < 9; i++) { // Número da linha
      for (j = 0; j < 9; j++) { // Número da coluna
        for (x = 0; x < 9; x++) { // Número da coluna auxiliar
          if (i*i + j*j == i*i + x*x || v[i][j] == 0) { // Se i² + j² igual a i² + x² (a mesma posição está sendo lida) ou o valor nesta posição da matriz for 0, o laço será passado 
      ...
      for (j = 0; j < 9; j++) { // Número da coluna
        for (x = 0; x < 9; x++) { // Número da coluna auxiliar
          for (y = 0; y < 9; y++) { // Número da linha auxiliar            
            if (i*i + j*j == j*j + y*y || v[i][j] == 0) { // Se i² + j² igual a j² + y² (a mesma posição está sendo lida) ou o valor nesta posição da matriz for 0, o laço será passado 
              continue; // Passa o laço
            }
        ...
        for (j = 0; j < 3; j++) { // Número da coluna
          for (x = c1-3; x < c1; x++) { // Número da linha auxiliar, aumenta 3 do inicial e 3 do limite a cada laço do while
            for (y = 0; y < 3; y++) { // Número da coluna auxiliar
              if (i*i + j*j == y*y + x*x || v[i][j] == 0) { // Se i² + j² igual a y² + x² (a mesma posição está sendo lida) ou o valor nesta posição da matriz for 0, o laço será passado 
                continue; // Passa o laço
              }
